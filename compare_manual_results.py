#!/usr/bin/env python3
"""
Compare asPLS Gaussian peak-fitting results against manually processed peak data.
...
- Mean row-wise relative height error can be unstable for small manual heights.
  Therefore this script also reports a normalized aggregate height error:
      sum(abs(asPLS_height - manual_height)) / sum(abs(manual_height))
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fitted-peaks",
        type=Path,
        default=Path("peak_fitting_results/erni_fitting_outputs/fitted_peaks.csv"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("manual_comparison_results"),
    )
    parser.add_argument(
        "--centre-tolerance",
        type=float,
        ...
        for reference_centre, manual_height in zip(
            ...
            if not sample_fits.empty:
                ...
                if centre_error <= centre_tolerance:
                    ...
                    row.update(
                        {
                            "aspls_centre_cm1": fitted_centre,
                            "aspls_height": fitted_height,
...
def create_comparison_plots(
    ...
    if not centre_rows.empty:
        ...
        ax.set_ylabel("asPLS fitted peak centre (cm$^{-1}$)")
        ax.set_title(
            f"Manual vs asPLS peak centres — {title_suffix}"
        )
        ax.legend()
    ...
    height_rows = rows.dropna(
        subset=["manual_height", "aspls_height"]
    )
    ...
    if not height_rows.empty:
        ...
        ax.set_ylabel("asPLS fitted peak height")
        ax.set_title(
            f"Manual vs asPLS peak heights — {title_suffix}"
        )
        ax.legend()
...
def main() -> None:
    ...
    print("=" * 72)
    print("asPLS manual comparison completed")
    print("=" * 72)
