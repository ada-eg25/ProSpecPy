#!/usr/bin/env python3
"""
Compare arPLS Gaussian peak-fitting results against manually processed peak data.
...
- Mean row-wise relative height error can be unstable for small manual heights.
  Therefore this script also reports a normalized aggregate height error:
      sum(abs(arPLS_height - manual_height)) / sum(abs(manual_height))
"""
        ...
        for reference_centre, manual_height in zip(
            ...
            if not sample_fits.empty:
                ...
                if centre_error <= centre_tolerance:
                    ...
                    row.update(
                        {
                            "arpls_centre_cm1": fitted_centre,
                            "arpls_height": fitted_height,
...
def create_comparison_plots(
    ...
    if not centre_rows.empty:
        ...
        ax.set_ylabel("arPLS fitted peak centre (cm$^{-1}$)")
        ax.set_title(
            f"Manual vs arPLS peak centres — {title_suffix}"
        )
        ax.legend()
    ...
    height_rows = rows.dropna(
        subset=["manual_height", "arpls_height"]
    )
    ...
    if not height_rows.empty:
        ...
        ax.set_ylabel("arPLS fitted peak height")
        ax.set_title(
            f"Manual vs arPLS peak heights — {title_suffix}"
        )
        ax.legend()
...
def main() -> None:
    ...
    print("=" * 72)
    print("arPLS manual comparison completed")
    print("=" * 72)
